...
Kandev показывает эти пути в интерфейсе как «potential repeated agent run» —
предупреждение о цикле, а не об ошибке. Каждый цикл замыкается только через
`on_turn_start`, то есть через ваше сообщение: сам по себе он не крутится, и
один круг стоит ровно одного намеренного действия человека. Круг с
`Human Review` — четыре хода агента (`Review Fixes`, `Final Verification`,
`Documentation`, `Draft PR`), круг с `Plan Approval` — два (`Planning`,
`Plan Review`).

Роли, которые от этого повторяются, обязаны быть к повтору готовы:
