...
## Возврат назад на доработку

Возврат делается вручную: написать замечание в задаче и перетащить карточку
назад. Вход в колонку перетаскиванием запускает её `on_enter` так же, как
обычный переход, поэтому агент стартует сам.

| Откуда | Куда тащить | Почему туда |
|---|---|---|
...
| `Human Review`, `Done` | `Review Fixes` | оттуда правка снова идёт через `Final Verification`, `Documentation` и `Draft PR` |

Возврат на `Review Fixes` пересекает сброс контекста, поэтому роль читает
разговор задачи через `get_task_conversation_kandev` и обращается с
возражением человека как с находкой: запись в `review-fixes.md`, закрыта или
объяснена.

Роли, которые от возврата повторяются, обязаны быть к повтору готовы:
`Review Fixes`, `Final Verification`, `Plan Review` и `Solution Synthesis`
ведут «Номер круга» и читают свой прошлый артефакт; `Draft PR` при
существующем PR обновляет его, а не открывает второй; `Documentation` читает
свой прошлый файл, чтобы не переделывать уже исправленное.

Автоматический возврат по сообщению (`on_turn_start`) здесь стоял и убран —
причина в решении 21.
