...
**Проверено: технической необходимости в этом нет.** `buildWorkflowPrompt`
(`orchestrator/task_operations.go:1920`) собирает промпт заново при каждом
запуске агента на шаге и безусловно подставляет блок
`## Workflow instructions` из промпта уровня цепочки перед промптом шага.
`reset_agent_context` чистит историю разговора, но следующее сообщение
строится этой же функцией — значит общий промпт доезжает до любого шага,
включая шаги со сброшенным контекстом.

Так что наш `@custom-artifact-protocol` на уровне цепочки работает, и
дублировать его по шагам не нужно. Почему авторы Kandev всё же дублируют —
остаётся их выбором; возможно, встроенные шаблоны написаны раньше, чем
появился промпт уровня цепочки.
...
1. ~~Проверить, доезжает ли workflow-level prompt после
   `reset_agent_context`.~~ **Проверено: доезжает.** Общий контракт можно
   держать на уровне цепочки, дублировать по шагам не нужно.
