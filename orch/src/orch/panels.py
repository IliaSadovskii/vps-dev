...
def _what_to_decide(db: Db, task) -> str:
    ...
    if reason == "chain_broken":
        ...
        return (
            f"Замороженная цепочка задачи не читается, и двигать задачу по "
            f"ней движок не станет.{tail} Починить цепочку можно только "
            f"руками: `orch task show {task['id']}` покажет её и ошибку, "
            f"править — `.orch/{task['id']}/chain.yml` в рабочей копии. "
            "«Закрыть задачу» работает и так: ветка с коммитами останется."
        )
