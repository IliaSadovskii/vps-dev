...
def test_чужой_артефакт_даёт_только_итог(ctx):
    ...
    # Роль обязана знать, что видит огрызок и что осталось в файле: на T37
    # План прочёл 24 строки `scoping.md` из 337 и не знал о «Правилах
    # проекта» и «Тестах и проверках».
    assert "только первый раздел файла" in text
    assert "`## Подробности`, `## Тесты и проверки`" in text
    assert "открой файл" in text


def test_файл_без_итога_назван_невклеенным(ctx):
    (ctx.task_dir / "artifacts" / "one.md").write_text(
        "# Заметки\n\nпросто текст без разделов\n", encoding="utf-8"
    )
    text = build(ctx)
    assert "Содержание не вклеено" in text
    assert "просто текст без разделов" not in text
...
def test_без_ворот_промпт_говорит_об_этом(ctx):
    ...
    # «После остановки правки вноси…» рядом с «задача не встанет» — два
    # утверждения, спорящих друг с другом; роль без ворот ждала разговора.
    assert "После остановки" not in text

    ctx.gate_after = True
    с_воротами = build(ctx)
    assert "задача встанет и будет ждать владельца" in с_воротами
    assert "После остановки правки владельца вноси" in с_воротами
