...
1. Проверить, доезжает ли workflow-level prompt до шага после
   `reset_agent_context`. Если нет, весь `@custom-artifact-protocol` не доедет
   до ровно тех ролей, которым он нужнее всего. Авторы Kandev дублируют общий
   блок в каждый шаг, а не выносят на уровень цепочки — возможно, именно
   поэтому.
